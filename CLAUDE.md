...
- **The bridge** (`bridge/amv-bridge.mjs`) is a zero-dependency daemon somebody
  ...
  Commands and connectors get an allowed list of environment variables
  (`--share-environment` lifts it) and run with the credential stores under home
  hidden (`--no-fence` lifts it): per process with bubblewrap on Linux, and on
  macOS the whole bridge re-starts under `sandbox-exec`. The fence is claimed
  only after a canary proves it hides something, and reported as `fence` to the
