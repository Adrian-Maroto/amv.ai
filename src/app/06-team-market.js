...
const AMVTeam = {
  ...
  /* Returns { team, existing }. The server answers a second create with the
     team the account already owns rather than making another one, so the
     caller has to be able to tell the difference - "Team created!" over a team
     that was already there is a claim about something that did not happen. */
  async create(name){ const r=await AMV_API._fetch('/team/create',{method:'POST',body:JSON.stringify({name})}); const d=await r.json(); if(d.error) throw new Error(d.error); this._cache=d.team; return { team:d.team, existing:!!d.existing }; },
  ...
  /* null, not [] - an empty presence list renders as "everyone is offline", and
     the drawing code falls back to showing every member greyed out. A dropped
     heartbeat would therefore have told somebody their whole team had left. */
  async heartbeat(){ try{ const r=await AMV_API._fetch('/team/presence',{method:'POST',body:JSON.stringify({ping:Date.now()})}); const d=await r.json(); return Array.isArray(d.present)?d.present:null; }catch(e){ return null; } },
  _presenceTimer:null,
  startPresence(onUpdate){
    if(!this.enabled()) return; this.stopPresence();
    /* A beat that could not be read leaves the last known presence on screen
       rather than blanking it. It re-runs in 25 seconds either way. */
    const beat=async()=>{ const present=await this.heartbeat(); if(present && onUpdate) try{ onUpdate(present); }catch(e){} };
...
function _renderTeamCreate(vc){
  ...
  on($('team-create-btn'),'click',async()=>{
    ...
    try{
      const res=await AMVTeam.create(name);
      toast(res.existing?'You already have a team - opening it.':'Team created!','success');
      renderTeamView();
    }
