...
AMVConnectors.register({
  ...
  actions: {
    do: {
      ...
      async run(args){
        const base = (typeof loadStr === 'function' && loadStr('amv_browser_service')) || '';
        if(!base){
          const e = new Error('Web automation needs the browser service. Deploy it and set it in Settings, then this runs automatically.');
        ...
        const r = await fetch(base.replace(/\/$/, '') + '/v1/browser/run', {
          ...
          body: JSON.stringify({ url: args.url, goal: args.goal, data: args.data || {} })
        });
  ...
  async execute(resolved, opts){
    ...
    for(let i = 0; i < resolved.length; i++){
      ...
      try{
        ...
        const blocker = e && e.code ? { code:e.code, need:e.message, how:e.message } : null;
